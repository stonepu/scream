...
namespace shining
{
    //typedef Callback<void, uint32_t> CwndCallback;
    typedef std::function<void(uint32_t)> CwndCallback;
...
static const float kGainUp = 1.1f;
...
static const int kTargetBitrateHistSize = 100; //原来是3
...
public:
    ...
    */
    ...
    void SetCwndCallback(CwndCallback callback_) { m_cwndCallback = callback_; }
    uint32_t mySsrc; //for test


private:
    CwndCallback m_cwndCallback;
    /*
    ...
    */
    ...
    uint64_t sRtt_us;
    float sRtt;
    volatile uint32_t ackedOwd;
    volatile uint32_t baseOwd;

    uint32_t baseOwdHist[kBaseOwdHistSize];
    int baseOwdHistPtr;
    volatile uint32_t baseOwdHistMin;
    float queueDelay;
    ...
    * ECN-CE
    */
    volatile bool ecnCeEvent;

    /*
    * Fast start
    */
    volatile bool inFastStart;

    /*
    * Transmission scheduling
    */
    uint64_t paceInterval_us;
    volatile float paceInterval;
    volatile float rateTransmittedAvg;
