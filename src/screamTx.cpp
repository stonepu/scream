...
//            mss = std::max((int)mss, sizeOfNextRtp);
//            if (!openWindow)
//                cwndMin = 2 * mss;
...
//        */
//        while (!stream->lossList.empty()) {
//            if (stream->lossList.front() < highestSeqNr - 200) {
//                stream->lossList.pop_front();
//            }
//            else {
//                break;
//            }
...
//                    */
//                    stream->lossList.push_back(seqNrExt);
//                    if (time_us - lastLossEventT_us > sRtt_us && lossBeta < 1.0f) {
//                            //lossEvent = true;
//
//                        float lossRate = 1.0 * stream->lossList.size() / 200;
//                        if (lossRate > 0.1 || getQueueDelayTrend() > 0.1) {
//                            lossEvent = true;
//                            I_LOG("loss rate: {}", lossRate);
//                        }
//                        else {
//                            I_LOG("detect loss but do nothing, loss rate: {}", lossRate);
//                        }
//                    }
//                    stream->bytesLost += tmp->size;
//                    tmp->isUsed = false;
...
//            }
//
//            queueDelayTrendMem = std::max(queueDelayTrendMem * 0.98f, (float)queueDelayTrend);
//
//            /*
...
//
//            float alpha = std::max((float)queueDelayTrend, cautiousPacing);
//            maxBytesInFlight =
...
//            */
//            int streamId;
//            auto stream = getStream(mySsrc, streamId);
//
//            float lossRate = streamId != -1 ? 1.0f * stream->lossList.size() / 200 :  0.11;
//            if (getQueueDelayTrend() < 0.1 && lossRate < 0.1) {
//                I_LOG("loss but do nothing with cwnd, lossRate: {}", lossRate);
//            }
//            else {
//                cwnd = std::max((int)cwndMin, (int)(lossBeta * cwnd));
//                I_LOG("update cwnd: {}", cwnd);
//            }
//            lossEvent = false;
...
//            */
//            if (isL4s) {
//                cwnd = std::max((int)cwndMin, (int)((1.0f - l4sAlpha / 2.0f) * cwnd));
//            }
//            else {
//                cwnd = std::max((int)cwndMin, (int)(ecnCeBeta * cwnd));
//            }
//            ecnCeEvent = false;
//            lastCongestionDetectedT_us = time_us;
//
...
//                        for (int n = 0; n < nStreams; n++) {
//                            Stream* tmp = streams[n];
//                            tmp->targetBitrate = std::max((float)tmp->minBitrate,
//                                std::min((float)(tmp->maxBitrate), tmp->targetBitrate * rateAdjustFactor));
//                        }
//                    }
//                    lastCongestionDetectedT_us = time_us;
//
...
//            baseOwdHistMin = UINT32_MAX;
//            for (int n = 0; n < kBaseOwdHistSize; n++)
//                baseOwdHistMin = std::min((uint32_t)baseOwdHistMin, baseOwdHist[n]);
//            /*
//            * _Very_ long periods of congestion can cause the base delay to increase
//            * with the effect that the queue delay is estimated wrong, therefore we seek to
//            * refresh the whole thing by deliberately allowing the network queue to drain
...
//                    streams[n]->targetBitrate = std::min((float)streams[n]->maxBitrate,
...
//        targetBitrate = std::min((float)maxBitrate, std::max((float)minBitrate, startBitrate_));
...
//                targetRateScale = std::min(1.25f, std::max(0.8f, (float)targetRateScale));
//            }
//        }
//
//        bytesTransmitted = 0;
...
//        targetBitrateHist[targetBitrateHistPtr] = std::min(br, (float)targetBitrate);
//        targetBitrateHistPtr = (targetBitrateHistPtr + 1) % kTargetBitrateHistSize;
//        targetBitrateI = std::min(br, (float)targetBitrate);
//        for (int n = 0; n < kTargetBitrateHistSize; n++) {
...
//            }
//            if (lossEventFlag) {
//                float lossRate = 1.0f * lossList.size() / 200;
//                if (parent->getQueueDelayTrend() < 0.1 && lossRate < 0.1) {
//                    //do nothing
//                    I_LOG("loss but do nothing, loss rate: {}", lossRate);
//                }
//                else if(parent->getQueueDelayTrend() > 0.02 || lossRate > 0.1) {
//                    I_LOG("loss and down targetRate");
//                    targetBitrate = std::max((float)minBitrate, targetBitrate * lossEventRateScale);
//                }
//
//            }
//            else if (ecnCeEventFlag) {
//                if (parent->isL4s) {
//                    targetBitrate = std::max((float)minBitrate, targetBitrate * (1.0f - parent->l4sAlpha / 4.0f));
//                    //updateTargetBitrateI(targetBitrate);
//                }
//                else {
//                    targetBitrate = std::max((float)minBitrate, targetBitrate * ecnCeEventRateScale);
//                }
//            }
//            lossEventFlag = false;
//            ecnCeEventFlag = false;
...
//                        */
//                        streams[n]->targetBitrate = std::max((float)streams[n]->minBitrate, streams[n]->targetBitrate * 0.9f);
//                    }
//                }
//            }
//
...
// Packet pacing reduces jitter
//取消pacing，这样的坏处是会增加
static const bool kEnablePacketPacing = false;
static const float kPacketPacingHeadRoom = 1.5f;
...
// Packet reordering margin (us)
static const uint64_t kReorderTime_us = 20000;
static const uint64_t kMinRtpQueueDiscardInterval_us = 1000000;  //1s
...
void ScreamTx::newMediaFrame(uint64_t time_us, uint32_t ssrc, int bytesRtp) {
    ...
    if (time_us - lastBaseDelayRefreshT_us < sRtt_us * 2) {
        ...
        * This funtion is executed very seldom so it should not affect overall experience too much
        */
        I_LOG("clear rtp queue");
        stream->rtpQueue->clear();
        stream->targetBitrate /= 2;
    }
    else {
        stream->bytesRtp += bytesRtp;
        /*
...
float ScreamTx::isOkToTransmit(uint64_t time_us, uint32_t &ssrc) {
    ...
    */
    ...
    if (stream == NULL) {
        ...
        * No RTP packets to transmit
        */
        W_LOG("no rtp packets to transmit");
        return -1.0f;
    }
    ssrc = stream->ssrc;
    ...
    */
    ...
    if (queueDelay < queueDelayTarget)
        ...
        exit = (bytesInFlight + sizeOfNextRtp) > cwnd;
    //if (exit) {
    //    //I_LOG("byteF: {}, nextSize: {}, cwnd: {}, mss: {}", bytesInFlight, sizeOfNextRtp, cwnd, mss);
    //    //exit = false;
    //    //I_LOG("exit");
    //}
    /*
...
void ScreamTx::incomingFeedback(uint64_t time_us,
    ...
    */
    detectLoss(time_us, txPackets, highestSeqNr, stream);
    //ecn需要硬件支持，不支持这种情况，不过检测到拥塞
    //I_LOG("incoming rtcp: ecnMarked: {}, stream->ecn: {}, time: {}，last: {}, rtt: {}", ecnCeMarkedBytes, stream->ecnCeMarkedBytes, time_us, lastLossEventT_us, sRtt_us);
    if (ecnCeMarkedBytes != stream->ecnCeMarkedBytes && time_us - lastLossEventT_us > sRtt_us) {
        ecnCeEvent = true;
        lastLossEventT_us = time_us;
    }
        ...
        if (txPackets[n].isUsed) {
            ...
            */
            if (tmp->seqNr == highestSeqNr) {
                ...
                */
                uint32_t qDel = estimateOwd(time_us);
                //I_LOG("owd: {}, baseOwd: {}", qDel, getBaseOwd());
                qDel -= getBaseOwd();
...
void ScreamTx::detectLoss(uint64_t time_us, struct Transmitted *txPackets, uint16_t highestSeqNr, Stream *stream) {
    ...
    */
    ...
    //            if (time_us - lastLossEventT_us > sRtt_us && lossBeta < 1.0f) {
    //                lossEvent = true;
    //                //I_LOG("loss packet time_us - lastLossEventT_us: {}, sRtt_us: {}", time_us - lastLossEventT_us, sRtt_us);
    //            }
    //            stream->bytesLost += tmp->size;
    //            tmp->isUsed = false;
    //            stream->repairLoss = true;
            ...
            if (tmp->timeTx_us + kReorderTime_us < stream->timeTxAck_us && !tmp->isAcked) {
                ...
                */
                ...
                if (time_us - lastLossEventT_us > sRtt_us && lossBeta < 1.0f) {
                    ...
                    float lossRate = 1.0 * stream->lossList.size() / 200;
                    if (lossRate > 0.1 || getQueueDelayTrend() > 0.2) {
                        lossEvent = true;
                        I_LOG("loss rate: {}",lossRate);
                    }
                    else {
                ...
                tmp->isUsed = false;
                //如果没有丢包，则把该包删除
                //I_LOG("seqNum to delete {}", tmp->seqNr);
                stream->rtpQueue->deleteAckedData(tmp->seqNr);
            }
        }
    }
...
float ScreamTx::getTargetBitrate(uint32_t ssrc) {
    int id;
    float bitrate = getStream(ssrc,id)->getTargetBitrate();
    return bitrate;
}
...
void ScreamTx::updateCwnd(uint64_t time_us) {
    ...
    */
    if ((time_us - lastAddToQueueDelayFractionHistT_us) >
        ...
        */
        int nIter = (int)(time_us - lastAddToQueueDelayFractionHistT_us) / kQueueDelayFractionHistInterval_us;
        //I_LOG("update cwnd: update queue delay fraction nIter:" << nIter);
        //I_LOG("\033[36mqueueDelay: " << queueDelay << "\033[0m, \033[4mtarget: " << queueDelayTarget << "\033[0m");
        for (int n = 0; n < nIter; n++) {
            queueDelayFractionHist[queueDelayFractionHistPtr] = getQueueDelayFraction();
            queueDelayFractionHistPtr = (queueDelayFractionHistPtr + 1) % kQueueDelayFractionHistSize;
        }
            ...
            */
            if (queueDelayTrend < 0.2) {
                ...
                */
                float bytesInFlightMargin = 1.5f;
                if (bytesInFlight*bytesInFlightMargin + bytesNewlyAcked > cwnd) {
                    cwnd += bytesNewlyAckedLimited * 1.5;
                    //I_LOG("cwnd changed up in fast start, increment: " << bytesNewlyAckedLimited << ", currCwnd: " << cwnd);
                }
            }
            else {
                inFastStart = false;
            ...
            if (offTarget > 0.0f) {
                ...
                */
                float bytesInFlightMargin = 1.2f;
                //I_LOG("\033[31m" << "bytesInFlight:" << bytesInFlight << ", bytesNewlyAcked:" << bytesNewlyAcked << ", result:" << bytesInFlight * bytesInFlightMargin + bytesNewlyAcked << ", CWND:" << cwnd << "\033[0m");
                if (bytesInFlight*bytesInFlightMargin + bytesNewlyAcked > cwnd) {
                                      //1.1
                    float increment = gainUp * offTarget * bytesNewlyAckedLimited * mss / cwnd;
                    cwnd += (int)(increment + 0.5f);
		            //I_LOG("cwnd changed, up, increment: " << increment << ", currCwnd: " << cwnd);
                }
            }
            else {
                /*
                ...
                */
                ...
                delta = std::min((int)delta, cwnd / 4);
                cwnd -= (int)(delta);
		        //I_LOG("\033[32mdown, delta: \033[0m" << delta<< ", curr: " << cwnd);
                ...
                */
                ...
                if (rateTotal < 1.0e5f) {
                    delta = delta / cwnd;
                    volatile float rateAdjustFactor = (1.0f - delta);
                    for (int n = 0; n < nStreams; n++) {
                        Stream *tmp = streams[n];
                        tmp->targetBitrate = std::max((float)tmp->minBitrate,
                            std::min((float)tmp->maxBitrate,
                            tmp->targetBitrate*rateAdjustFactor));
                        //I_LOG("\033[32m " << "stream update target bitrate: " << tmp->targetBitrate << ", delta:" << delta << "\033[0m");
                    }
                }
                lastCongestionDetectedT_us = time_us;
    ...
    */
    if (maxBytesInFlight > 5000) {
        //I_LOG("maxBytesInFlight: " << maxBytesInFlight);
        cwnd = std::min((int)cwnd, (int)maxBytesInFlight);
    }
    ...
    if (sRtt < 0.01f && queueDelayTrend < 0.1) {
        ...
        tmp = std::max(tmp, (int)(maxBytesInFlight*1.5f));
        cwnd = std::max((int)cwnd, tmp);
        //I_LOG("update cwnd in low delay, cwnd: " << cwnd);
    }
    cwnd = std::max(cwndMin, cwnd);
    ...
    * Make possible to enter fast start if OWD has been low for a while
    */
    //I_LOG("\033[32m" << "queueTrend: " << queueDelayTrend << ", timeUs:" << time_us << ", lastCongestionDetectedT_us: " << lastCongestionDetectedT_us << ", interval: " << 1.0 * (time_us - lastCongestionDetectedT_us) / 1e6 << "\033[0m");
    if (queueDelayTrend > 0.2) {
        lastCongestionDetectedT_us = time_us;
    }
    else if (time_us - lastCongestionDetectedT_us > 5000000 &&
    //else if (time_us - lastCongestionDetectedT_us > 2000000 && //by wrl
        !inFastStart && kEnableConsecutiveFastStart) {
        /*
        * The queue delay trend has been low for more than 5.0s, resume fast start
        */
    ...
    bytesNewlyAcked = 0;
    //I_LOG("final cwnd: " << cwnd);
    if (m_cwndCallback) {
        //float temp = (float)cwnd;
        m_cwndCallback(cwnd);
    }
}
    ...
    if (lastRateUpdateT_us != 0) {
        ...
        if (rateRtp > 0) {
            ...
            */
            ...
            targetRateScale *= (1.0f - alpha);
            targetRateScale += alpha*targetBitrate / rateRtp;
            targetRateScale = std::min(1.25f, std::max(0.5f, (float)targetRateScale));
            //I_LOG("update rate: targetBitrate: " << targetBitrate << " rateRtp: " << rateRtp << " targetRateScale: " << targetRateScale);
	}
    }
...
float ScreamTx::Stream::getTargetBitrate() {
    ...
    float rate = targetRateScale*targetBitrate;
    //I_LOG("targetScale: " << targetRateScale << ", targetRate: " << targetBitrate);
    /*
    * Video coders are strange animals.. In certain cases a very frequent rate requests can confuse the
    * rate control logic in the coder
    */
...
*/
void ScreamTx::Stream::updateTargetBitrateI(float br) {
    //I_LOG("screamTx.cc:1623:update target birate I");
    targetBitrateHist[targetBitrateHistPtr] = std::min(br, (float)targetBitrate);
    targetBitrateHistPtr = (targetBitrateHistPtr + 1) % kTargetBitrateHistSize;
    targetBitrateI = std::min(br, (float)targetBitrate);
    for (int n = 0; n < kTargetBitrateHistSize; n++) {
...
void ScreamTx::Stream::updateTargetBitrate(uint64_t time_us) {
    ...
    */
    float br = getMaxRate();
   // float rateRtpLimit = br;
    float rateRtpLimit = std::max((float)rateRtp, br);
    if (initTime_us == 0) {
        /*
        * Initialize if the first time
        */
    ...
    if (lossEventFlag || ecnCeEventFlag || rtpQueue->size() > 100) {
        /*
        * Loss event handling
        * Rate is reduced slightly to avoid that more frames than necessary
        * queue up in the sender queue
        */
        if (time_us - lastTargetBitrateIUpdateT_us > 2000000) {
            ...
            *  over fixed bandwidth or slowly varying links.
            */
            I_LOG("update target br in loss event");
            updateTargetBitrateI(br);
            lastTargetBitrateIUpdateT_us = time_us;
        }
        if (lossEventFlag) {
            //I_LOG("screamTx.cc:1673:update target bitrate: loss");
            targetBitrate = std::max((float)minBitrate, targetBitrate * lossEventRateScale);
        }
        else if (ecnCeEventFlag) {
            //I_LOG("screamTx.cc:1677:update target bitrate: ecn");
            if (parent->isL4s) {
                targetBitrate = std::max((float)minBitrate, targetBitrate*(1.0f - parent->l4sAlpha / 4.0f));
                //updateTargetBitrateI(targetBitrate);
            }
        ...
        if (time_us - lastBitrateAdjustT_us < kRateAdjustInterval_us) {
            //I_LOG("update target bitrate \033[31mnot changed \033[0m");
            return;
        }
        /*
        * A scale factor that is dependent on the inflection point
        ...
        */
        float rampUpSpeedTmp = std::min(rampUpSpeed, targetBitrate * 0.5f);
        if (parent->isCompetingFlows()) {
            //I_LOG("\033[31m" << "is competing flows" << "\033[0m");
            rampUpSpeedTmp *= 2.0f;
        }
        float rtpQueueDelay = rtpQueue->getDelay(time_us * 1e-6f);
        //I_LOG("screamTx.cc:1725:rampUpSpeedTmp: " << rampUpSpeedTmp << ", rtpQueueDelay: " << rtpQueueDelay);
                                //0.1
        if (rtpQueueDelay > maxRtpQueueDelay &&
            (time_us - lastRtpQueueDiscardT_us > kMinRtpQueueDiscardInterval_us)) {
            /*
            ...
            * throughput in the initial phase.
            */
            I_LOG("screamTx.cc:1733: rtp clear {}", rtpQueue->size());
            rtpQueue->clear();
            targetBitrate /= 2;
            rtpQueueDiscard = true;
            ...
            * No increase if the actual coder rate is lower than the target
            */
            if (targetBitrate > rateRtpLimit * 1.3f) {
                //I_LOG("in fast but not increment");
                increment = 0;
            }
            /*
            * Add increment
            */
            //I_LOG("screamTx.cc:1759 in fast start" << " increament: " << increment << ", targetBr:" << targetBitrate);
            targetBitrate += increment * 0.5;
            wasFastStart = true;
        }
        else {
            if (wasFastStart) {
                ...
                if (time_us - lastTargetBitrateIUpdateT_us > 2000000) {
                    ...
                    * congestion event is prolonged
                    */
                    //I_LOG("update tbrI in fast start");
                    updateTargetBitrateI(br);
                    lastTargetBitrateIUpdateT_us = time_us;
                }
            }
            ...
            */
            ...
            if (!parent->isL4s || parent->l4sAlpha < 0.001) {
                ...
                */              //0.1
                ...
                if (parent->isCompetingFlows())
                    scl *= 0.05f; 
                //I_LOG("\033[34m before increment: " << increment << "\033[0m,\033[35m br: " << br << "\033[0m, \033[32mtargetBitrate: " << targetBitrate << "\033[0m, txSizeBitsAvg:" << txSizeBitsAvg);
                //print("txSizeBitsAvg: " + std::to_string(txSizeBitsAvg));
                //print("increment: " + std::to_string(increment));
                                                        //0.2
                increment = increment*(1.0f - scl) - txQueueSizeFactor*txSizeBitsAvg;
                //I_LOG("scl: " << scl << ",\033[32m queueDelayTrend: \033[0m" << parent->getQueueDelayTrend() << ", increment: " << increment);
            }
            increment -= targetBitrate;
            //increment *= 0.2;
            //rtp队列中的bit减去上一帧的比特数
            ...
            if (increment > 0) {
                ...
                if (!parent->isCompetingFlows()) {
                    ...
                    */
                    increment *= sclI;
                    increment = std::min(increment, (float)(rampUpSpeedTmp*(kRateAdjustInterval_us * 1e-6)));
                    //I_LOG("not competing: increment > 0: increment: " << increment);
                }
                if (targetBitrate > rateRtpLimit*1.5f) {
                    /*
                    * Limit increase if the target bitrate is considerably higher than the actual 
                    ...
                    */
                    increment = 0;
                    //I_LOG("increment > 0: increment = 0");
                }
            }
            else {
                //increment小于0
                //I_LOG("increment < 0: rateRtp: " << rateRtp << ", tarBr: " << targetBitrate);
                /**
                  *有的时候由于延时低但是检测的带宽也低，不知道什么原因
                  */
                if (parent->getQueueDelayTrend() < 0.02f) {
                    //I_LOG("test queueDelay is too low, not decrea");
                    increment *= 0.2;
                }
            ...
            targetBitrate += increment;
            //I_LOG("increment: " << increment << " targetBr:" << targetBitrate);
        }
        lastBitrateAdjustT_us = time_us;
    }

    targetBitrate = std::min(maxBitrate, std::max(minBitrate, targetBitrate));
    //I_LOG("\033[31mfinal target rate: " << targetBitrate << "\033[0m");
}
...
void print(std::string str) {
    cout << "\033[35m" << str << "\033[0m" << endl;
}

void print(int content) {
    cout << "\033[35m" << content << "\033[0m" << endl;
}
